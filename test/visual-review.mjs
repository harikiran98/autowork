...
try {
  ...
  await page.getByLabel('Your name').fill('Visual Reviewer')
  await page.getByLabel('Workspace name').fill('Visual Studio')
  ...
  await page.getByText('Visual Studio', { exact: true }).first().waitFor()
  ...
  // Trigger a real reassignment and verify movement through an intermediate
  // point before the figure reaches the couch and takes a seated pose.
  const walkStart = await page.evaluate(() => window.reviewScene.getObjectByName('agent-a1').position.toArray())
  await page.locator('aside').getByRole('button', { name: 'Bench', exact: true }).click()
  await page.waitForTimeout(850)
  const walkMiddle = await page.evaluate(() => {
  ...
  await reduced.getByRole('dialog', { name: 'Create your workspace' }).waitFor()
  await reduced.getByLabel('Your name').fill('Reduced Motion')
  await reduced.getByLabel('Workspace name').fill('Reduced Studio')
  await reduced.getByRole('dialog', { name: 'Create your workspace' }).getByRole('button', { name: 'Enter workspace' }).click()
