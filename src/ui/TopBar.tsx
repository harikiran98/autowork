...
function Icon({ name }: { name: 'work' | 'files' | 'outputs' | 'labels' | 'sun' | 'moon' }) {
...
export function TopBar({ drawer, onDrawer, panelOpen = false, user, onSignOut }: {
  ...
  const outputs = completedTasks(agents).length + teamJobs.filter((job) => job.status === 'done' || job.status === 'error').length
